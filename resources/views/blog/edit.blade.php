...
<section class="create-hero">
    <div class="hero-content">
        ...
        <p>Update your thoughts, ideas &amp; stories with the world</p>
    </div>
</section>
...
<section class="form-section">
    <div class="container">
        <div class="form-card">
            ...
            <form action="{{ route('blog.update', $post->_id) }}" method="POST" enctype="multipart/form-data" id="blogEditForm">
                @csrf
                @method('PUT')
                ...
                <div class="field-group">
                    ...
                    <input type="text" id="post-title" name="title" class="form-control"
                           placeholder="Enter an engaging title…" value="{{ old('title', $post->title) }}" autocomplete="off">
                    @error('title')
                    ...
                    <select id="post-category" name="category" class="form-select">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->category_name }}"
                                {{ old('category', $post->category) === $cat->category_name ? 'selected' : '' }}>
                                {{ $cat->category_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="field-group">
                    ...
                    <div id="imagePreviewWrap">
                        <img id="imagePreview" src="{{ url($post->post_image) }}" alt="Image Preview">
                    </div>
                    @error('post_image')
                        <p class="err-msg">{{ $message }}</p>
                    @enderror
                ...
                <div class="field-group">
                    ...
                    <textarea id="post-content" name="content" class="form-control"
                              placeholder="Write your blog content here…" rows="7">{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <p class="err-msg">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="field-group">
                    ...
                    <select id="post-status" name="status" class="form-select">
                        <option value="1" {{ old('status', $post->status) == 1 ? 'selected' : '' }}>🟢 Active (Visible to everyone)</option>
                        <option value="0" {{ old('status', $post->status) == 0 ? 'selected' : '' }}>🔴 Inactive (Draft)</option>
                    </select>
                    @error('status')
                        <p class="err-msg">{{ $message }}</p>
                    @enderror
